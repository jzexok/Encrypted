...
local function FartHubLoad()
	...
	local GUI = Rayfield:CreateWindow({
		Name = "purna | bombsaken",
		Theme = "DarkBlue",
		LoadingTitle = "purna script hub is loading...",
		LoadingSubtitle = "PORNOPORNOCURBADEHSETZURNATAYLANDKIZLARI",
		Icon = "cat",
		Link = "",
		...
		KeySystem = true,
		KeySettings = {
			Title = "purna key system",
			Subtitle = "get the key from our discord server.",
			Note = "copied link to keyboard.",
			FileName = "purnahubkey",
			SaveKey = true,
			GrabKeyFromSite = false,
			Key = { "kurtlercantusethis" },
		},
	})
	...
	local function GetAssetList()
		local url = "https://api.github.com/repos/ivannetta/ShitScri"
		local assetList = {}
			...
			if Request then
				...
				if response and response.Body then
					...
					for _, item in ipairs(data.tree) do
						...
						then
							local rawUrl = "https://raw.githubusercontent.com/ivannetta/ShitS" .. item.path
	...
	local function ToggleFarts(state)
		...
		local function AddFart(object, color)
			...
			then
				...
				h.FillColor, h.FillTransparency, h.OutlineTransparency = color, 0.8, 0.6
			end
		end
	...
	local function NotifyFartsakeners(Player)
		...
		then
			Rayfield:Notify({
				Title = "another pURNAAAAAAA user",
				Content = (Player.Name .. " is also using purnascripthub!"),
				Duration = 10,
				Image = "cat",
			})
		...
		while true do
			for _, player in ipairs(game.Players:GetPlayers()) do
				local success, err = pcall(function()
					...
					if Pronouns.Value == "dih/lover" then
	...
	local function ToggleFart(state)
		...
		if not success then
			Rayfield:Notify({
				Title = "ballisticerrorstamina",
				Content = "your stupid ahh exe doseent supprt this.",
				Duration = 5,
				Image = "ban",
			})
			task.wait(5)
	...
	local function InitializeGUI()
		...
		-- Visuals Tab
		local VisualsSection = VisualsTab:CreateSection("highlights for survivors.")
		...
		-- Player Tab
		local PlayerSection = PlayerTab:CreateSection("stuff that blablabla.")
		...
		-- Generator Tab
		local GeneratorSection = GeneratorTab:CreateSection("tralelo tralala")
		...
		-- Blatant Tab
		BlatantTab:CreateSection("main and shit")
		...
		local FlingKillerButton = BlatantTab:CreateButton({
			Name = "fling killer",
			Description = "vay be. (killer needs to stay still)",
			Callback = function()
				FlingKiller()
			end,
		})
	...
	local function CheckAndDeleteAssets()
		local basePath = "shitttypcugother"
		local sigmaFilePath = "niggers.txt"
